...
local history = require("widgets.launcher.history")

local MAX_AMOUNT = 5

local get_all_apps = function()
  local all_apps = {}
...
local search_app = function(input)
  ...
  for i, app in ipairs(all_apps) do
    ...
    local input_in_description = app:get_description() ~= nil and
        string.find(string.lower(app:get_description()), string.lower(input))
